...
class Dot {
  ...
  updatePosition() {
    ...
    this.dot.style.transform = `translate(${this.posX}px, ${this.posY}px)`;
  }
  ...
  add_pos(x, y) {
    ...
    if (this.posY >= window.innerHeight) this.posY = 1;
    else if (this.posY <= 0) this.posY = window.innerHeight - 1;

    if (this.posX >= window.innerWidth) this.posX = 1;
    else if (this.posX <= 0) this.posX = window.innerWidth - 1;
...
function animateDots() {
  dots.forEach(dot => dot.add_pos(deltaPosition_x, deltaPosition_y));
